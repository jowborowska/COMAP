...
def xs_with_model(figure_name, k, xs_mean_l, xs_mean_u, xs_sigma_l, xs_sigma_u):
   ...
   ax2.errorbar(k, diff_mean / error, error /error, fmt='o', label=r'$\tilde{C}_{diff}(k)$', color='teal')
   ax2.errorbar(k, sum_mean / error, error /error, fmt='o', label=r'$\tilde{C}_{sum}(k)$', color='purple')
