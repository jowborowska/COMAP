...
def xs_with_model(figure_name, k, xs_mean_l, xs_mean_u, xs_sigma_l, xs_sigma_u):
   ...
   #ax1.plot(k_th, k_th * ps_th_nobeam * 10, '--', label=r'$10 \times kP_{Theory}(k)$', color='dodgerblue')
   #ax1.plot(k_th, k_th * ps_copps_nobeam * 5, 'g--', label=r'$5 \times kP_{COPPS}$ (shot)')
   ax1.set_ylabel(r'$k\tilde{C}_{\mathrm{diff}}(k)$ [$\mu$K${}^2$ Mpc${}^2$]', fontsize=16)
   ax1.set_ylim(-lim*2, lim*2)              # ax1.set_ylim(0, 0.1)
   ax1.set_xlim(0.04,0.5)
   ax1.set_xscale('log')
   ax1.grid()
   ax1.set_xlabel(r'$k$ [Mpc${}^{-1}$]', fontsize=16)
   labnums = [0.02, 0.04,0.1, 0.2, 0.4]
   labels = ['0.02', '0.04', '0.1','0.2', '0.4']
   #ax1.set_xticks(labnums, minor=True)
   ...
   ax2.plot(k, 0 * xs_mean_l, 'k', alpha=0.4)
   #ax2.set_ylabel(r'$\tilde{C}(k) / \sigma_\tilde{C}$')
   ax2.set_ylabel(r'$\tilde{C}_{\mathrm{diff}}(k) / \sigma_\tilde{C}$', fontsize=16)
...
def call_all(mapname, split):
   ...
   xs_with_model('xs_mean_' + mapname + '_null.pdf', kl, xs_mean_l, xs_mean_u, xs_sigma_l, xs_sigma_u)
   print ("Created files:")
