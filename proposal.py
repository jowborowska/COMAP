...
def xs_with_model_3fields(figure_name, k, xs_mean2, xs_mean6, xs_mean7, xs_sigma2, xs_sigma6, xs_sigma7, scan_strategy):
   ...
   if scan_strategy == 'ces':
      titlename = 'CES scans, night time data'
   if scan_strategy == 'liss':
      titlename = 'Lissajous scans, night time data'
   ...
   ax1.errorbar(k6, k * xs_mean6 / (transfer(k)*transfer_Nils(k)), k * xs_sigma6 / (transfer(k)*transfer_Nils(k)), fmt='o', label=r'co6', color='teal', zorder=2)
   ax1.errorbar(k7, k * xs_mean7 / (transfer(k)*transfer_Nils(k)), k * xs_sigma7 / (transfer(k)*transfer_Nils(k)), fmt='o', label=r'co7', color='purple', zorder=3)
   ax1.errorbar(k, k * xs_mean2 / (transfer(k)*transfer_Nils(k)), k * xs_sigma2 / (transfer(k)*transfer_Nils(k)), fmt='o', label=r'co2', color='indianred', zorder=1)
   ...
   ax2.errorbar(k6, xs_mean6 / xs_sigma6, xs_sigma6/xs_sigma6, fmt='o', label=r'co6', color='teal', zorder=2)
   ax2.errorbar(k7, xs_mean7 / xs_sigma7, xs_sigma7/xs_sigma7, fmt='o', label=r'co7', color='purple', zorder=3)
   ax2.errorbar(k, xs_mean2 / xs_sigma2, xs_sigma2/xs_sigma2, fmt='o', label=r'co2', color='indianred', zorder=1)
...
xs_with_model_3fields('xs_mean_' + 'ces' + '_' + 'dayn' + '.pdf', k2c, mean2c, mean6c, mean7c, sigma2c, sigma6c, sigma7c, 'ces')
...
xs_with_model_3fields('xs_mean_' + 'liss' + '_' + 'dayn' + '.pdf', k2l, mean2l, mean6l, mean7l, sigma2l, sigma6l, sigma7l, 'liss')
